"""Config flow for Samsung Custom Appliances."""
...
class SmartThingsConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_auth(self, user_input=None):
        ...
        safe_scope = urllib.parse.quote(SMARTTHINGS_SCOPES, safe="")
        auth_link = (
            f"{OAUTH_AUTH_URL}?client_id={safe_client_id}"
            f"&response_type=code&redirect_uri={safe_redirect_uri}&scope={safe_scope}"
        )
        ...
        return self.async_show_form(
            ...
            errors=errors,
            description_placeholders={"auth_link": auth_link},
        )
